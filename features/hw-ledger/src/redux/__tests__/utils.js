...
export function setup({ dependencies = [] } = {}) {
  ...
  const emitBluetoothStatus = (data) => handleEvent('bluetoothStatus', data)
  const emitBluetoothScanning = (data) => handleEvent('bluetoothScanning', data)

  return { ...redux, store, emitBluetoothStatus, emitBluetoothScanning }
}
