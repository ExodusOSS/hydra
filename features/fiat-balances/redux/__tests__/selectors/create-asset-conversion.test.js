...
describe('createAssetConversion', () => {
  const rates = {
    USD: {
  ...
  const assertEthConversion = ({ currency, expected }) => {
    ...
    const selector = selectors.fiatBalances.createAssetConversion

    emitAssets(assetsToEmit)
    emitRates(rates)

    const conversion = selector(assets.ethereum.name)(store.getState())
