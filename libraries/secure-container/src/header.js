...
import varstruct, { Bound, UInt8, UInt32BE, VarString } from 'varstruct'
...
export function encode(header) {
  return struct.encode(header)
}

export function serialize(header) {
  const buf = Buffer.alloc(HEADER_LEN_BYTES)
...
export function create({ appName, appVersion } = { appName: '', appVersion: '' }) {
  return {
    magic: MAGIC,
    version: 0,
    reserved: 0,
