...
test('readFile / writeFile (with blobkey)', async (t) => {
  ...
  const [writeFileErr, { metadata, blobKey }] = await aw(writeFile)(testFile, secretMessage, {
    passphrase,
  })
  t.ifError(writeFileErr, 'no write file error')
...
test('readFile returns valid blobKey and metadata', async (t) => {
  ...
  const secretMessage = Buffer.from('Hello, lets meet at 10 PM to plan our secret mission!', 'utf8')
  const secretMessage2 = Buffer.from(
    'Hello, lets meet at 10 AM to plan our secret mission!',
    'utf8'
  )
  const passphrase = Buffer.from('opensesame', 'utf8')
  ...
  const [writeFileErr2] = await aw(writeFile)(testFile, secretMessage2, {
    metadata,
    blobKey,
    overwrite: true,
  })
