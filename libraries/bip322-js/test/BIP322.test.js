...
describe('BIP322 Test', () => {
  ...
  it('Draft correct BIP-322 toSpend transaction', () => {
    ...
    // Assert
    expect(emptyStringToSpendTx.getId().toLowerCase()).toEqual(
      'c5680aa69bb8d860bf82d4e9cd3504b55dde018de765a91bb566283c545a99a7'
    )
    expect(helloWorldToSpendTx.getId().toLowerCase()).toEqual(
  ...
  it('Draft correct BIP-322 toSign transaction', () => {
    ...
    const emptyStringToSignTxSigned = emptyStringToSignTx
      .signAllInputs(testPrivateKey)
      .finalizeAllInputs()
      .extractTransaction()
    const helloWorldToSignTxSigned = helloWorldToSignTx
      .signAllInputs(testPrivateKey)
      .finalizeAllInputs()
      .extractTransaction()
    // Assert the transaction ID is as expected
    expect(emptyStringToSignTxSigned.getId().toLowerCase()).toEqual(
      '1e9654e951a5ba44c8604c4de6c67fd78a27e81dcadcfe1edf638ba3aaebaed6'
    )
    expect(helloWorldToSignTxSigned.getId().toLowerCase()).toEqual(
  ...
  it('Throw error when trying to encode witness from unsigned PSBT', () => {
    ...
    // Assert
    expect(result).toThrow('Cannot encode empty witness stack.')
  })
})
