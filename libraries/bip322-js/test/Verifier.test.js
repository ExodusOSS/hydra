...
describe('Verifier Test', () => {
  it('Can verify legacy P2PKH signature', () => {
    ...
    const resultWrongMessageTestnet = Verifier.verifySignature(
      addressTestnet,
      messageWrong,
      signature
    ) // Wrong message - should be false
    const resultWrongAddress = Verifier.verifySignature(addressWrong, message, signature) // Wrong address - should be false
    const resultWrongAddressTestnet = Verifier.verifySignature(
      addressWrongTestnet,
      message,
      signature
    ) // Wrong address - should be false

    // Assert
    expect(resultCorrect).toBeTruthy()
    expect(resultCorrectTestnet).toBeTruthy()
    expect(resultWrongMessage).toBeFalsy()
    expect(resultWrongMessageTestnet).toBeFalsy()
    expect(resultWrongAddress).toBeFalsy()
    expect(resultWrongAddressTestnet).toBeFalsy()
  })
  ...
  it('Can verify legacy BIP-137 signature from P2SH-P2WPKH, P2WPKH, and P2TR address', () => {
    ...
    const signature =
      'IAtVrymJqo43BCt9f7Dhl6ET4Gg3SmhyvdlW6wn9iWc9PweD7tNM5+qw7xE9/bzlw/Et789AQ2F59YKEnSzQudo=' // Signed by public key "02f7fb07050d858b3289c2a0305fbac1f5b18233798665c0cbfe133e018b57cafc"
    ...
    const p2wpkhMainnetValidResult = Verifier.verifySignature(
      p2wpkhMainnetValid,
      message,
      signature
    )
    const p2wpkhTestnetValidResult = Verifier.verifySignature(
      p2wpkhTestnetValid,
      message,
      signature
    )
    ...
    const p2pkhMainnetInvalidResult = Verifier.verifySignature(
      p2pkhMainnetInvalid,
      message,
      signature
    )
    const p2pkhTestnetInvalidResult = Verifier.verifySignature(
      p2pkhTestnetInvalid,
      message,
      signature
    )
    const p2shMainnetInvalidResult = Verifier.verifySignature(
      p2shMainnetInvalid,
      message,
      signature
    )
    const p2shTestnetInvalidResult = Verifier.verifySignature(
      p2shTestnetInvalid,
      message,
      signature
    )
    const p2wpkhMainnetInvalidResult = Verifier.verifySignature(
      p2wpkhMainnetInvalid,
      message,
      signature
    )
    const p2wpkhTestnetInvalidResult = Verifier.verifySignature(
      p2wpkhTestnetInvalid,
      message,
      signature
    )
    const p2trMainnetInvalidResult = Verifier.verifySignature(
      p2trMainnetInvalid,
      message,
      signature
    )
    const p2trTestnetInvalidResult = Verifier.verifySignature(
      p2trTestnetInvalid,
      message,
      signature
    )
    ...
    const invalidAddressTestnetResult = Verifier.verifySignature(
      invalidAddressTestnet,
      message,
      signature
    )

    // Assert
    expect(p2pkhMainnetValidResult).toBeTruthy()
    expect(p2pkhTestnetValidResult).toBeTruthy()
    expect(p2shMainnetValidResult).toBeTruthy()
    expect(p2shTestnetValidResult).toBeTruthy()
    expect(p2wpkhMainnetValidResult).toBeTruthy()
    expect(p2wpkhTestnetValidResult).toBeTruthy()
    expect(p2trMainnetValidResult).toBeTruthy()
    expect(p2trTestnetValidResult).toBeTruthy()

    expect(p2pkhMainnetInvalidResult).toBeFalsy()
    expect(p2pkhTestnetInvalidResult).toBeFalsy()
    expect(p2shMainnetInvalidResult).toBeFalsy()
    expect(p2shTestnetInvalidResult).toBeFalsy()
    expect(p2wpkhMainnetInvalidResult).toBeFalsy()
    expect(p2wpkhTestnetInvalidResult).toBeFalsy()
    expect(p2trMainnetInvalidResult).toBeFalsy()
    expect(p2trTestnetInvalidResult).toBeFalsy()

    expect(invalidAddressResult).toBeFalsy()
    expect(invalidAddressTestnetResult).toBeFalsy()
  })
  ...
  it('Can verify and falsify BIP-322 signature for P2SH-P2WPKH address', () => {
    ...
    const resultCorrectTestnet = Verifier.verifySignature(
      addressTestnet,
      messageHelloWorld,
      signature
    ) // Everything correct
  ...
  it('Can verify and falsify BIP-322 signature for P2WPKH address', () => {
    ...
    const resultEmptyValidTestnet = Verifier.verifySignature(
      addressTestnet,
      messageEmpty,
      signatureEmpty
    )
    const resultHelloWorldValid = Verifier.verifySignature(
      address,
      messageHelloWorld,
      signatureHelloWorld
    )
    const resultHelloWorldValidTestnet = Verifier.verifySignature(
      addressTestnet,
      messageHelloWorld,
      signatureHelloWorld
    )
    const resultHelloWorldValidII = Verifier.verifySignature(
      address,
      messageHelloWorld,
      signatureHelloWorldAlt
    )
    const resultHelloWorldValidIITestnet = Verifier.verifySignature(
      addressTestnet,
      messageHelloWorld,
      signatureHelloWorldAlt
    )
    // Correct addresses but incorrect signature
    const resultHelloWorldInvalidSig = Verifier.verifySignature(
      address,
      messageEmpty,
      signatureHelloWorld
    ) // Mixed up the signature and message - should be false
    const resultHelloWorldInvalidSigTestnet = Verifier.verifySignature(
      addressTestnet,
      messageEmpty,
      signatureHelloWorld
    ) // Mixed up the signature and message - should be false
    const resultEmptyInvalidSig = Verifier.verifySignature(
      address,
      messageHelloWorld,
      signatureEmpty
    ) // Mixed up the signature and message - should be false
    const resultEmptyInvalidSigTestnet = Verifier.verifySignature(
      addressTestnet,
      messageHelloWorld,
      signatureEmpty
    ) // Mixed up the signature and message - should be false
    // Incorrect addresses
    const resultEmptyInvalidAddress = Verifier.verifySignature(
      addressWrong,
      messageEmpty,
      signatureEmpty
    ) // Wrong address - should be false
    const resultEmptyInvalidAddressTestnet = Verifier.verifySignature(
      addressWrongTestnet,
      messageEmpty,
      signatureEmpty
    ) // Wrong address - should be false
    const resultHelloWorldInvalidAddress = Verifier.verifySignature(
      addressWrong,
      messageHelloWorld,
      signatureHelloWorld
    ) // Wrong address - should be false
    const resultHelloWorldInvalidAddressTestnet = Verifier.verifySignature(
      addressWrongTestnet,
      messageHelloWorld,
      signatureHelloWorld
    ) // Wrong address - should be false
    ...
    // Assert
    expect(resultEmptyValid).toBeTruthy()
    expect(resultEmptyValidTestnet).toBeTruthy()
    expect(resultHelloWorldValid).toBeTruthy()
    expect(resultHelloWorldValidTestnet).toBeTruthy()
    expect(resultHelloWorldValidII).toBeTruthy()
    expect(resultHelloWorldValidIITestnet).toBeTruthy()
    expect(resultHelloWorldInvalidSig).toBeFalsy()
    expect(resultHelloWorldInvalidSigTestnet).toBeFalsy()
    expect(resultEmptyInvalidSig).toBeFalsy()
    expect(resultEmptyInvalidSigTestnet).toBeFalsy()
    expect(resultEmptyInvalidAddress).toBeFalsy()
    expect(resultEmptyInvalidAddressTestnet).toBeFalsy()
    expect(resultHelloWorldInvalidAddress).toBeFalsy()
    expect(resultHelloWorldInvalidAddressTestnet).toBeFalsy()
  })
  ...
  it('Can verify and falsify BIP-322 signature for single-key-spend P2TR address using SIGHASH_ALL flag', () => {
    ...
    const resultWrongAddress = Verifier.verifySignature(
      addressWrong,
      messageHelloWorld,
      signatureHelloWorld
    ) // Wrong address - should be false
  ...
  it('Can verify and falsify BIP-322 signature for single-key-spend P2TR address using SIGHASH_DEFAULT flag', () => {
    ...
    const toSignTxSigned = toSignTx
      .signAllInputs(testPrivateKeyTweaked, [bitcoin.Transaction.SIGHASH_DEFAULT])
      .finalizeAllInputs()
    // Extract the signature
    const signature = BIP322.encodeWitness(toSignTxSigned)
  ...
  it('Refuse to verify P2WSH transaction', () => {
    ...
    const signature =
      ''
  ...
  it('Reject verification from malformed address', () => {
    ...
    const signatureP2PKH =
      'H9L5yLFjti0QTHhPyFrZCT1V/MMnBtXKmoiKDZ78NDBjERki6ZTQZdSMCtkgoNmp17By9ItJr8o7ChX0XxY91nk=' // Correctly encoded P2PKH signature
    const signatureP2WPKH =
      'AkcwRAIgM2gBAQqvZX15ZiysmKmQpDrG83avLIT492QBzLnQIxYCIBaTpOaD20qRlEylyxFSeEA2ba9YOixpX8z46TSDtS40ASECx/EgAxlkQpQ9hYjgGu6EBCPMVPwVIVJqO4XCsMvViHI=' // Correctly encoded P2WPKH signature
    const signatureP2TR =
      'AUHd69PrJQEv+oKTfZ8l+WROBHuy9HKrbFCJu7U1iK2iiEy1vMU5EfMtjc+VSHM7aU0SDbak5IUZRVno2P5mjSafAQ==' // Correctly encoded P2TR signature

    // Act
    const resultP2PKH = Verifier.verifySignature.bind(
      Verifier,
      malformP2PKH,
      message,
      signatureP2PKH
    )
    const resultP2WPKHInP2SH = Verifier.verifySignature.bind(
      Verifier,
      malformP2WPKHInP2SH,
      message,
      signatureP2WPKH
    )
    const resultP2WPKH = Verifier.verifySignature.bind(
      Verifier,
      malformedP2WPKH,
      message,
      signatureP2WPKH
    )
    const resultP2Tr = Verifier.verifySignature.bind(
      Verifier,
      malformedP2TR,
      message,
      signatureP2TR
    ...
    // Assert
    expect(resultP2PKH).toThrow() // Throw by bitcoinjs-message library
    expect(resultP2WPKHInP2SH).toThrow() // Throw by bitcoinjs-lib
    expect(resultP2WPKH).toThrow() // Throw by helper/Address
    expect(resultP2Tr).toThrow() // Throw by helper/Address
  })
  ...
  it('Reject Schnorr signature with incorrect length', () => {
    ...
    const signatureHelloWorldExtraByte = Buffer.concat([
      signatureHelloWorldDeserialized,
      Buffer.from([0xff]),
    ])
    ...
    // Act
    const result = Verifier.verifySignature.bind(
      Verifier,
      address,
      messageHelloWorld,
      signatureHelloWorldExtraByteSerialized
    ) // Schnorr signature with incorrect length

    // Assert
    expect(result).toThrow('Invalid Schnorr signature provided.')
  })
  ...
  it('Reject signature signed using invalid SIGHASH', () => {
    ...
    const signatureAnyOneCanPay = BIP322.encodeWitness(
      BIP322.buildToSignTx(toSpendTx.getId(), scriptPubKey, false, internalPublicKey)
        ...
        .signAllInputs(testPrivateKeyTweaked, [bitcoin.Transaction.SIGHASH_ANYONECANPAY])
        .finalizeAllInputs()
    )
    const signatureInputMask = BIP322.encodeWitness(
      BIP322.buildToSignTx(toSpendTx.getId(), scriptPubKey, false, internalPublicKey)
        .updateInput(0, { sighashType: bitcoin.Transaction.SIGHASH_INPUT_MASK })
        .signAllInputs(testPrivateKeyTweaked, [bitcoin.Transaction.SIGHASH_INPUT_MASK])
        .finalizeAllInputs()
    )
    const signatureNone = BIP322.encodeWitness(
      BIP322.buildToSignTx(toSpendTx.getId(), scriptPubKey, false, internalPublicKey)
        .updateInput(0, { sighashType: bitcoin.Transaction.SIGHASH_NONE })
        .signAllInputs(testPrivateKeyTweaked, [bitcoin.Transaction.SIGHASH_NONE])
        .finalizeAllInputs()
    )
    const signatureOutputMask = BIP322.encodeWitness(
      BIP322.buildToSignTx(toSpendTx.getId(), scriptPubKey, false, internalPublicKey)
        .updateInput(0, { sighashType: bitcoin.Transaction.SIGHASH_OUTPUT_MASK })
        .signAllInputs(testPrivateKeyTweaked, [bitcoin.Transaction.SIGHASH_OUTPUT_MASK])
        .finalizeAllInputs()
    )
    const signatureSingle = BIP322.encodeWitness(
      BIP322.buildToSignTx(toSpendTx.getId(), scriptPubKey, false, internalPublicKey)
        .updateInput(0, { sighashType: bitcoin.Transaction.SIGHASH_SINGLE })
        .signAllInputs(testPrivateKeyTweaked, [bitcoin.Transaction.SIGHASH_SINGLE])
        .finalizeAllInputs()
    )

    // Act
    const resultAnyOneCanPay = Verifier.verifySignature.bind(
      Verifier,
      address,
      messageHelloWorld,
      signatureAnyOneCanPay
    )
    const resultInputMask = Verifier.verifySignature.bind(
      Verifier,
      address,
      messageHelloWorld,
      signatureInputMask
    )
    const resultNone = Verifier.verifySignature.bind(
      Verifier,
      address,
      messageHelloWorld,
      signatureNone
    )
    const resultOutputMask = Verifier.verifySignature.bind(
      Verifier,
      address,
      messageHelloWorld,
      signatureOutputMask
    )
    const resultSingle = Verifier.verifySignature.bind(
      Verifier,
      address,
      messageHelloWorld,
      signatureSingle
    ...
    // Assert
    expect(resultAnyOneCanPay).toThrow(
      'Invalid SIGHASH used in signature. Must be either SIGHASH_ALL or SIGHASH_DEFAULT.'
    )
    expect(resultInputMask).toThrow(
      'Invalid SIGHASH used in signature. Must be either SIGHASH_ALL or SIGHASH_DEFAULT.'
    )
    expect(resultNone).toThrow(
      'Invalid SIGHASH used in signature. Must be either SIGHASH_ALL or SIGHASH_DEFAULT.'
    )
    expect(resultOutputMask).toThrow(
      'Invalid SIGHASH used in signature. Must be either SIGHASH_ALL or SIGHASH_DEFAULT.'
    )
    expect(resultSingle).toThrow(
      'Invalid SIGHASH used in signature. Must be either SIGHASH_ALL or SIGHASH_DEFAULT.'
    )
  })
})
