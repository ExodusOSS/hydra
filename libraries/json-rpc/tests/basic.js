...
test('callMethod - sync error without cause', function(t) {
  ...
  client
    ...
    .catch(err => {
      ...
      t.deepEqual(err.cause, undefined)
    })
})
...
test('ping-pong: test client-server bidirectional communication', function(t) {
  ...
  server.exposeMethods({
    ping: (counter, message) => {
      if (counter) {
        return server.callMethod('pong', [
          counter - 1,
          `${message}-server${counter}`
        ])
      }
      return `${message}-finish`
    }
  ...
  client
    ...
    .then(result =>
      t.equal(result, 'start-server4-client3-server2-client1-finish')
    )
    .catch(t.fail)
  ...
  server
    ...
    .then(result =>
      t.equal(result, 'start-client4-server3-client2-server1-finish')
    )
    .catch(t.fail)
})
...
test('exposing non-function', function(t) {
  ...
  client
    .callMethod('test')
    .then(t.fail)
    .catch(err => t.equal(err.code, -32603))
})

test('constructor parameter validation', function(t) {
  t.plan(2)
  t.throws(() => new RPC(), 'TRANSPORT_REQUIRED')
  t.throws(() => new RPC({}), 'TRANSPORT_REQUIRED')
})
...
test('ignores messages with same id after reply was already received', function(t) {
  ...
  transport.write = message =>
    t.equal(
      message,
      '{"jsonrpc":"2.0","method":"testCall","params":[42],"id":0}'
    )
...
test('returns raw response with callMethodWithRawResponse', function(t) {
  ...
  client.callMethodWithRawResponse('test', [])
    .then((result) => {
  ...
  client.callMethodWithRawResponse('errormethod', [])
    .then((result) => {
      const expected = {
        jsonrpc: '2.0',
        id: 1,
        error: { code: -32603, message: 'woopsie', data: { } },
      }
