/* eslint-disable no-labels, valid-typeof */
...
export default function conformToMask(rawValue = emptyString, mask = emptyArray, config = {}) {
  if (!isArray(mask)) {
    ...
    if (typeof mask === strFunction) {
      ...
      throw new TypeError('Text-mask:conformToMask; The mask property must be an array.')
    }
  }
  ...
  if (suppressGuide && isAddition === false) {
    ...
    if (indexOfLastFilledPlaceholderChar === null) {
      ...
      conformedValue = conformedValue.slice(0, Math.max(0, indexOfLastFilledPlaceholderChar + 1))
    }
  }
