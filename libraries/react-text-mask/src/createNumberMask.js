/* eslint-disable valid-typeof,unicorn/explicit-length-check,unicorn/prefer-spread */
...
function convertToMask(strNumber) {
  return strNumber
    .split(emptyString)
    .map((char) => (digitRegExp.test(char) ? new RegExp(char) : char))
}
...
export default function createNumberMask({
  ...
  function numberMask(rawValue = emptyString) {
    ...
    if (rawValue === emptyString || (rawValue[0] === prefix[0] && rawValueLength === 1)) {
      return [...prefix.split(emptyString), digitRegExp].concat(suffix.split(emptyString))
    }

    if (rawValue === decimalSymbol && allowDecimal) {
      return [...prefix.split(emptyString), '0', decimalSymbol, digitRegExp].concat(
