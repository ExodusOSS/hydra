...
// data from https://github.com/satoshilabs/slips/blob/90e24c9311063dc4f264c9b49528c26afb8c0d44/slip-0010.md
...
const tests1 = [
  // ignoring this vector because bip32-path thinks its invalid
  // {
  //   chain: 'm/',
...
tests1.forEach((v, i) => {
  test('slip-0010 tests 1 vector ' + i, (t) => {
    t.plan(2)
    const derived = HdKey.fromSeed(seed1).derive(v.chain)
    t.equals(derived.key.toString('hex'), v.private)
    t.equals(derived.chainCode.toString('hex'), v.chainCode)
  })
})

tests2.forEach((v, i) => {
  test('slip-0010 tests 2 vector ' + i, (t) => {
    t.plan(2)
    const derived = HdKey.fromSeed(seed2).derive(v.chain)
    t.equals(derived.key.toString('hex'), v.private)
    t.equals(derived.chainCode.toString('hex'), v.chainCode)
  })
})

test('multiple derive', (t) => {
  t.plan(2)
  const derived = HdKey.fromSeed(seed1).derive("m/0'/1'/2'").derive("m/2'/1000000000'")
  t.equals(
    derived.key.toString('hex'),
    '8f94d394a8e8fd6b1bc2f3f49f5c47e385281d5c17e65324b0f62483e37e8793'
  )
  t.equals(
    derived.chainCode.toString('hex'),
    '68789923a0cac2cd5a29172a475fe9e0fb14cd6adb5ad98a3fa70333e7afa230'
  )
})

test('unhardened path', (t) => {
  t.plan(2)
  let err
  try {
    HdKey.fromSeed(seed1).derive('m/0/1')
  } catch (e) {
    err = e
  }

  t.ok(err, 'should throw')
  t.equals(err ? err.message : '', 'invalid path: contains unhardened segment')
})
...
test('NaN path', (t) => {
  ...
  let err
  try {
    HdKey.fromSeed(seed1).derive("m/pizza'")
  } catch (e) {
    err = e
  }

  t.ok(err, 'should throw')
  t.equals(err ? err.message : '', 'Invalid input')
})
