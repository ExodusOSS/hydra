...
test('string "txId" is required', (t) => {
  t.throws(() => PersonalNote.fromJSON({} as never), /txId/)
  t.throws(() => PersonalNote.fromJSON({ txId: 123 } as never), /txId/)
  t.end()
})
  ...
  ;[
    ...
    for (const key in json) {
      t.equal(
        ...
        reexported[key as keyof typeof reexported],
        `property ${key} survived fromJSON / toJSON`
      )
    }
  })
...
test('equals()', (t) => {
  ...
  t.equal(transferNote.equals(undefined as never), false, 'undefined is not equal')
