...
export default class UtxoCollection {
  ...
  static fromArray(utxoArray, { addressMap, ...options } = {}) {
    ...
    const utxos = utxoArray.map((utxo) => {
      ...
      return {
        ...utxo,
        value: isNumberUnit(utxo.value) ? utxo.value : currency.parse(utxo.value),
      }
    })
  ...
  static fromJSON(json, { currency }) {
    if (typeof json === 'string') json = JSON.parse(json)

    const utxoData = {}
    Object.keys(json).forEach((address) => {
      const addr = new Address(json[address].address, { path: json[address].path })

      // sort by smallest -> to largest
      const sortedUtxos = sortBy(json[address].utxos, (utxo) =>
  ...
  getAddressUtxos(address) {
    ...
    return UtxoCollection.fromArray(this._data[address], { ...this })
  }
  ...
  merge(utxoCol) {
    let currentUtxos = this // eslint-disable-line unicorn/no-this-assignment
  ...
  addUtxo(utxo, addressMap) {
    return UtxoCollection.fromArray([...this.toArray(), utxo], { addressMap })
  }

  setAddressUtxos(address, utxos, { addressMap } = {}) {
  ...
  toJSON() {
    ...
    ;[...this.addresses].forEach((addr) => {
      ...
      jsonMap[address] = {
        address,
        path: addr.meta.path,
        utxos: sortedUtxos.map((utxo) => {
          utxo = { ...utxo, value: utxo.value.toString() }
          delete utxo.address // eslint-disable-line @exodus/mutable/no-param-reassign-prop-only
          return utxo
        }),
      }
    })
  ...
  // does not overwrite exiting data
  filter(fn) {
    return UtxoCollection.fromArray(this.toArray().filter(fn), { ...this })
  }

  // overwrite existing data
  unionOverwrite(utxos) {
    const arr1 = this.toArray()
    const arr2 = utxos.toArray()
  ...
  [Symbol.iterator] = function* symbolIterator() {
