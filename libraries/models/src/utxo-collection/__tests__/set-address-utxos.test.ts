...
type FixtureKey = keyof typeof fixture

test('set utxos for an address', function (t) {
  const addresses = Object.keys(fixture)
  const addr1Utxos = { [addresses[0]!]: clone(fixture[addresses[0] as FixtureKey]) }
  const addr2Utxos = { [addresses[1]!]: clone(fixture[addresses[1] as FixtureKey]) }
