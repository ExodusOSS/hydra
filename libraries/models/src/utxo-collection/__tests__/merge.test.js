...
test('replace address utxos', function (t) {
  ...
  t.is(utxos.value.toString(), '15 BTC', 'initial value')
  t.is(
  ...
  t.is(utxos2.value.toString(), '10.5 BTC', 'value of address utxos to merge')
  t.is(
  ...
  t.is(utxosFinal.value.toString(), '22.5 BTC', 'merged value')
  t.is(
...
test('add new utxos', function (t) {
  ...
  t.is(utxos3.value.toString(), '7.7 BTC', 'value of address utxos to merge')
  t.is(utxos3.toArray().length, 2, '2 total utxos')

  const utxosFinal = utxos.merge(utxos3)
  t.is(utxosFinal.value.toString(), '22.7 BTC', 'merged value')
