...
import { sameDate as _sameDateTxs } from './fixtures/index.js'
...
test('fromArray() should have deterministic sort when two txs have the same date', (t) => {
  ...
  const txset2 = TxSet.fromArray(sameDateTxs.reverse()) // eslint-disable-line sonarjs/no-misleading-array-reverse
...
test('fromArray() should convert undefined / null / non-array value', (t) => {
  ...
  // @ts-expect-error -- not adding type for this behavior, we should throw
