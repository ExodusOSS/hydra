...
test('greater than with positive numbers', function () {
  ...
  expect(b1.gt(b2)).toBeFalsy()
  expect(b2.gt(b1)).toBeFalsy()

  expect(b3.gt(b1)).toBeTruthy()
  expect(b1.gt(b3)).toBeFalsy()

  expect(b4.gt(b3)).toBeTruthy()
  expect(b3.gt(b4)).toBeFalsy()
})
...
test('greater than with negative numbers', function () {
  ...
  expect(b0.gt(b1)).toBeTruthy()
  expect(b1.gt(b0)).toBeFalsy()

  expect(b1.gt(b2)).toBeTruthy()
  expect(b2.gt(b1)).toBeFalsy()
