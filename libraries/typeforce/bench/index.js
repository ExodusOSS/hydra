...
const fixtures = [...tests.valid, ...tests.invalid]
...
fixtures.forEach(function (f) {
  ...
  if (f.exception) {
    suite.add('local(e)#' + tdescription, function () {
      try {
        local(type, value, f.strict)
      } catch {}
    })
    suite.add('  npm(e)#' + tdescription, function () {
      try {
        npm(type, value, f.strict)
      } catch {}
    })
    suite.add('local(c, e)#' + tdescription, function () {
      try {
        local(ctype, value, f.strict)
      } catch {}
    })
    suite.add('  npm(c, e)#' + tdescription, function () {
      try {
        npm(ctype, value, f.strict)
      } catch {}
    })
  } else {
    suite.add('local#' + tdescription, function () {
      local(type, value, f.strict)
    })
    suite.add('  npm#' + tdescription, function () {
      npm(type, value, f.strict)
    })
    suite.add('local(c)#' + tdescription, function () {
      local(ctype, value, f.strict)
    })
    suite.add('  npm(c)#' + tdescription, function () {
      npm(ctype, value, f.strict)
    })
  }
  ...
  suite.on('complete', function () {
    console.log(
      '\n> Fastest is' +
        (' ' + this.filter('fastest').pluck('name').join(' | ')).replace(/\s+/u, ' ') +
        '\n'
    )
  })
